...
def vhs(img, layer, add_border, add_messy, add_glitch, glitch_y,
        down_interpol, up_interpol, filename):
    ...
    rand_levels = random.randint(50, 255)

    # Apply levels to luminance layer
    adjust_levels(luminance_layer, rand_levels, rand_levels, rand_levels)

    # Apply levels to chrominance layer
    adjust_levels(chrominance_layer, rand_levels, rand_levels, rand_levels)

    # Apply Noise
    pdb.plug_in_noisify(img, layer, 0, 0.2, 0.2, 0.2, 0.2)
    ...
    final_layer = pdb.gimp_image_merge_down(img, chrominance_layer, CLIP_TO_IMAGE)
...
def scale_to_pal(image, down_interpol, up_interpol):
    ...
    offsetX = (image.width - PAL_WIDTH) / 2
    offsetY = (image.height - PAL_HEIGHT) / 2 

    # Crop image
    pdb.gimp_image_crop(image, PAL_WIDTH, PAL_HEIGHT, offsetX, offsetY)
...
def add_vhs_glitch(image, layer, start_y = 146):
    ...
    shift_values = (19, 19, 16, 41, 12, 11)
